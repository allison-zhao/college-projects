...
//    video = new Capture(canvas, cameras[15]);
  ...
  void run()
  {
    if (video.available())
    {
      ...
      video.loadPixels();
      preFrame.loadPixels();
      arraycopy(video.pixels, preFrame.pixels);
  ...
  boolean checkHit(int xPos, int yPos, int w, int h)
  {
    ...
    for (int x = xPosStart; x < xPosEnd; x++)
    {
      for (int y = yPosStart; y < yPosEnd; y++)
      {
        ...
        float previousRed   = red(preFrame.pixels[location]);
        float previousGreen = green(preFrame.pixels[location]);
        float previousBlue  = blue(preFrame.pixels[location]);
